...
public class CalendarNumbersView extends View {
    ...
    public interface DateSelectionListener {
        void onDateSelected(Calendar selectedDate, Calendar enddCalendar);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        paint.setTextSize(getResources().getDimensionPixelSize(
                R.dimen.calendar_default_text_size));
        textColor = getResources()
                .getColor(R.color.calendar_default_text_color);
        inactiveTextColor = getResources().getColor(
                R.color.calendar_default_inactive_text_color);
        selectionTextColor = getResources().getColor(
                R.color.calendar_default_selection_text_color);
        cellPadding = getResources().getDimensionPixelSize(
                R.dimen.calendar_default_cell_padding);
        cellBackgroundColor = getResources().getColor(
                R.color.calendar_default_cell_background_color);
        cellSelectionBackgroundColor = getResources().getColor(
                R.color.calendar_default_cell_selection_background_color);
        dayNamesTextColor = getResources().getColor(
                R.color.calendar_default_day_names_cell_text_color);
        dayNamesBackgroundColor = getResources().getColor(
                R.color.calendar_default_day_names_cell_background_color);

        TypedArray ta = getContext().obtainStyledAttributes(attrs,
                R.styleable.CalendarNumbersView);
        if (ta != null) {
            paint.setTextSize(ta.getDimensionPixelSize(
                    R.styleable.CalendarNumbersView_fontSize,
                    (int) paint.getTextSize()));
            textColor = ta.getColor(R.styleable.CalendarNumbersView_textColor,
                    textColor);
            inactiveTextColor = ta.getColor(
                    R.styleable.CalendarNumbersView_inactiveTextColor,
                    inactiveTextColor);
            selectionTextColor = ta.getColor(
                    R.styleable.CalendarNumbersView_selectionTextColor,
                    selectionTextColor);
            cellPadding = ta.getDimensionPixelSize(
                    R.styleable.CalendarNumbersView_cellPadding, cellPadding);
            cellBackgroundColor = ta.getColor(
                    R.styleable.CalendarNumbersView_cellBackgroundColor,
                    cellBackgroundColor);
            cellSelectionBackgroundColor = ta
                    .getColor(
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int calculatedWidth = quadCellSideWidth
                * shownMonth.getActualMaximum(Calendar.DAY_OF_WEEK)
                + getPaddingLeft() + getPaddingRight();
        int calculatedHeight = quadCellSideWidth * MAX_WEEKS_IN_MONTH
                + getPaddingTop() + getPaddingBottom();
        if (showDayNames) {
            calculatedHeight += quadCellSideWidth;
        }
        int minimumWidth = Math
                .max(getSuggestedMinimumWidth(), calculatedWidth);
        int minimumHeight = Math.max(getSuggestedMinimumHeight(),
                calculatedHeight);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        _cachedCellSideWidth = (w - getPaddingRight() - getPaddingLeft())
                / shownMonth.getActualMaximum(Calendar.DAY_OF_WEEK);
        _cachedCellSideHeight = (h - getPaddingTop() - getPaddingBottom())
                / MAX_WEEKS_IN_MONTH;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (showDayNames) {
            ...
            for (int col = 0; col < _calendar
                    .getActualMaximum(Calendar.DAY_OF_WEEK); col++) {
                String name = _calendar.getDisplayName(Calendar.DAY_OF_WEEK,
                        Calendar.SHORT, locale);
                ;
                drawCell(canvas, -1, col, dayNamesTextColor,
                        dayNamesBackgroundColor, name, true);
        ...
        for (int row = 0; row < MAX_WEEKS_IN_MONTH; row++) {
            for (int col = 0; col < _calendar
                ...
                int backgroundColor;
                if (enddate != null
                        && _calendar.get(Calendar.DAY_OF_YEAR) == enddate
                        .get(Calendar.DAY_OF_YEAR)
                        && _calendar.get(Calendar.YEAR) == enddate
                        .get(Calendar.YEAR)) {
                    textColor = selectionTextColor;
                    backgroundColor = cellSelectionBackgroundColor;
                } else if (selectedDate != null
                        && _calendar.get(Calendar.DAY_OF_YEAR) == selectedDate
                        .get(Calendar.DAY_OF_YEAR)
                        && _calendar.get(Calendar.YEAR) == selectedDate
                        .get(Calendar.YEAR)) {
                    textColor = selectionTextColor;
                    backgroundColor = cellSelectionBackgroundColor;
                } else if (selectedDate != null
                        ...
                        (_calendar.get(Calendar.YEAR) > selectedDate.get(Calendar.YEAR)
                                && _calendar.get(Calendar.DAY_OF_YEAR) + 366 >= selectedDate.get(Calendar.DAY_OF_YEAR)))
                        && (_calendar.get(Calendar.DAY_OF_YEAR) <= enddate
                        .get(Calendar.DAY_OF_YEAR)
                        ||
                        (_calendar.get(Calendar.YEAR) < enddate
                                .get(Calendar.YEAR) && _calendar.get(Calendar.DAY_OF_YEAR) <= (enddate
                                .get(Calendar.DAY_OF_YEAR) + 366))
                )
                        && _calendar.get(Calendar.YEAR) >= selectedDate
                        .get(Calendar.YEAR)
                        && _calendar.get(Calendar.YEAR) <= enddate
                    ...
                    if (_calendar.get(Calendar.MONTH) == shownMonth
                            .get(Calendar.MONTH)) {
                ...
                drawCell(canvas, row, col, textColor, backgroundColor, str,
                        false);
                _calendar.add(Calendar.DAY_OF_MONTH, 1);
            }
        }
    ...
    private void drawCell(Canvas canvas, int row, int col, int textColor,
                          int backgroundColor, String str, boolean bold) {
        ...
        if (bold) {
            ...
            if (_defaultTypeface == null) {
                _defaultTypeface = Typeface.create(Typeface.DEFAULT,
                        Typeface.NORMAL);
            }
            paint.setTypeface(_defaultTypeface);
        }
        paint.setTextAlign(Paint.Align.CENTER);
        canvas.drawText(str, _rect.left + _cachedCellSideWidth / 2f,
                _rect.top + _cachedCellSideHeight / 2f + _textHeight / 2f
                        - paint.getFontMetrics().descent / 2, paint);
    }
    ...
    private void setCalendarToFirstVisibleDay(Calendar calendar) {
        ...
        if (firstDayInWeek > firstDayOfWeekOfCurrentMonth) {
            shift = -(firstDayOfWeekOfCurrentMonth
                    + calendar.getActualMaximum(Calendar.DAY_OF_WEEK) - firstDayInWeek);
        } else {
    ...
    private void getRectForCell(int col, int row, Rect outRect) {
        ...
        outRect.set(getPaddingLeft() + col * _cachedCellSideWidth,
                getPaddingTop() + row * _cachedCellSideHeight, getPaddingLeft()
                        + col * _cachedCellSideWidth + _cachedCellSideWidth,
                getPaddingTop() + row * _cachedCellSideHeight
                        + _cachedCellSideHeight);
    }
    ...
    private void selectDayAt(float x, float y) {
        ...
        if (selectedDate == null || (enddate != null && selectedDate != null)) {
            selectedDate = Calendar.getInstance();
            selectedDate.setTime(_calendar.getTime());
            enddate = null;
            if (listener != null) {
                listener.onDateSelected(selectedDate, enddate);
            }

        } else if (selectedDate != null && enddate == null) {
            enddate = Calendar.getInstance();
            enddate.setTime(_calendar.getTime());
            if (enddate.getTimeInMillis() < selectedDate.getTimeInMillis()) {
