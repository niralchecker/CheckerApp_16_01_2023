...
import com.mor.sa.android.activities.AcraActivity;
...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
	...
	ErrorReporter(Application context, SharedPreferences prefs, boolean enabled) {
		...
		final String initialConfiguration = ConfigurationCollector
			...
			ApplicationHelper.registerActivityLifecycleCallbacks(context,
					new ActivityLifecycleCallbacksCompat() {
						...
						public void onActivityCreated(Activity activity,
							...
							if (ACRA.DEV_LOGGING)
								ACRA.log.d(ACRA.LOG_TAG, "onActivityCreated "
										+ activity.getClass());
						...
						public void onActivityStarted(Activity activity) {
							if (ACRA.DEV_LOGGING)
								ACRA.log.d(ACRA.LOG_TAG, "onActivityStarted "
										+ activity.getClass());
						}
						...
						public void onActivityResumed(Activity activity) {
							if (ACRA.DEV_LOGGING)
								ACRA.log.d(ACRA.LOG_TAG, "onActivityResumed "
										+ activity.getClass());
						}
						...
						public void onActivityPaused(Activity activity) {
							if (ACRA.DEV_LOGGING)
								ACRA.log.d(ACRA.LOG_TAG, "onActivityPaused "
										+ activity.getClass());
						}
						...
						public void onActivityStopped(Activity activity) {
							if (ACRA.DEV_LOGGING)
								ACRA.log.d(ACRA.LOG_TAG, "onActivityStopped "
										+ activity.getClass());
						}
						...
						public void onActivityDestroyed(Activity activity) {
							if (ACRA.DEV_LOGGING)
								ACRA.log.i(ACRA.LOG_TAG, "onActivityDestroyed "
										+ activity.getClass());
						}
					});
		}
	...
	public void uncaughtException(Thread t, Throwable e) {
		try {
			...
			Log.e(ACRA.LOG_TAG, "ACRA caught a " + e.getClass().getSimpleName()
	...
	private void endApplication() {
		if (ACRA.getConfig().mode() == ReportingInteractionMode.SILENT
				|| (ACRA.getConfig().mode() == ReportingInteractionMode.TOAST && ACRA
						.getConfig().forceCloseDialogAfterToast())) {
			// If using silent mode, let the system default handler do it's job
			// and display the force close dialog.
			mDfltExceptionHandler.uncaughtException(brokenThread,
					unhandledThrowable);
		} else {
			...
			Log.e(LOG_TAG, mContext.getPackageName() + " fatal error : "
					+ unhandledThrowable.getMessage(), unhandledThrowable);
	...
	public void setEnabled(boolean enabled) {
		Log.i(ACRA.LOG_TAG, "ACRA is " + (enabled ? "enabled" : "disabled")
	...
	public void checkReportsOnApplicationStart() {
		...
		if ((ACRA.getConfig().mode() == ReportingInteractionMode.NOTIFICATION || ACRA
				.getConfig().mode() == ReportingInteractionMode.DIALOG)
		...
		if (filesList != null && filesList.length > 0) {
			...
			ReportingInteractionMode reportingInteractionMode = ACRA
					.getConfig().mode();
	...
	private void handleException(Throwable e,
		...
		final CrashReportData crashReportData = crashReportDataFactory
				.createCrashData(e, forceSilentReport, brokenThread);
		...
		if (reportingInteractionMode == ReportingInteractionMode.SILENT
			...
			Log.d(ACRA.LOG_TAG,
					"Notification will be created on application start.");
		}
		...
		new Thread() {
			...
			public void run() {
				...
				if (showDirectDialog) {
					...
					// after its death.
					Log.d(ACRA.LOG_TAG,
							"About to create DIALOG from #handleException");
	...
	void notifyDialog(String reportFileName) {
		...
		try{
		Intent i = new Intent(mContext,AcraActivity.class);
		i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		i.putExtra("crash",sValue);
...
//		mContext.startActivity(dialogIntent);
	...
	private void notifySendReport(String reportFileName) {
		...
		final Intent notificationIntent = new Intent(mContext,
				CrashReportDialog.class);
		Log.d(LOG_TAG, "Creating Notification for " + reportFileName);
		notificationIntent.putExtra(ACRAConstants.EXTRA_REPORT_FILE_NAME,
				reportFileName);
		final PendingIntent contentIntent = PendingIntent.getActivity(mContext,
				mNotificationCounter++, notificationIntent,
				PendingIntent.FLAG_UPDATE_CURRENT);

//		notification.setLatestEventInfo(mContext, contentTitle, contentText,
//				contentIntent);

		final Intent deleteIntent = new Intent(mContext,
				CrashReportDialog.class);
		deleteIntent.putExtra(ACRAConstants.EXTRA_FORCE_CANCEL, true);
		final PendingIntent pendingDeleteIntent = PendingIntent.getActivity(
				mContext, -1, deleteIntent, 0);
	...
	private void saveCrashReportFile(String fileName, CrashReportData crashData) {
		try {
			sValue = crashData.toString();
			crashConent = crashData.toString();
			...
			for (int iFile = 0; iFile < filesList.length - nbOfLatestToKeep; iFile++) {
				...
				if ((isReportApproved && deleteApprovedReports)
					...
					if (!fileToDelete.delete()) {
						Log.e(ACRA.LOG_TAG, "Could not delete report : "
								+ fileToDelete);
					}
				}
			}
	...
	public void setDefaultReportSenders() {
		...
		if (!"".equals(conf.mailTo())) {
			Log.w(LOG_TAG, mApplication.getPackageName()
