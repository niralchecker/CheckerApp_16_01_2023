...
public class NewUITitleGroupHelper {
	...
	private View getDropDownView(ArrayList<Answers> listAnswers,
		...
		spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view, int i,
				...
				dataQuestionGroup thisQGroup=((GroupSpinner)spinner).getThisQgroup();
...

		{
			...
			if (altTitles != null)
				for (int i = 0; i < altTitles.size(); i++) {
					if (altTitles.get(i).getTitleText() != null
							&& title != null
							&& altTitles.get(i).getTitleText().toLowerCase()
									.trim().equals(title.toLowerCase().trim())) {
						altTitle = altTitles.get(i).getAltTitle();
						altTitle = ConvertTextCodetoText(altTitle);
					}
				}
...
thisQGroup.setTextbox(textbox);
	...
	public LinearLayout prepareGroupLayout(Display display, Helper helper,
		...
		MainLayout.setLayoutParams(new TableRow.LayoutParams(
				TableRow.LayoutParams.WRAP_CONTENT, display.getWidth()));
		...
		if (this.titles == null || this.titles.size() == 0) {
			tblRows = simpleTwoRowQuestions(4, tblRows, MainLayout);
		} else {
			tblRows = simpleTitleRows(4, tblRows, MainLayout);
		}
			...
			try {
				...
				if (Helper.comapreString(questionObject.getObjectType(), "4")
					...
					if (questionObject.getListAnswers() != null
						...
						viewId = getViewId(viewId);
						questionnaireLayout = addRow(objectCount, tblRows,
								questionnaireLayout, true, null, "titleView");
					}
					...
					if (questionObject.getListAnswers() != null
						...
						if (!questionObject.getDisplayType().equals("1")
							...
							for (ansCount = 0; ansCount < questionObject
								...
								Answers answers = questionObject
										.getListAnswers().get(ansCount);
								if (answers.getAnswerDisplayCondition() != null) {
									Boolean b = IsObjectdisplaybyCondition(answers
											.getAnswerDisplayCondition());
									if (!b)
	...
	private RelativeLayout addRow(int counter, ArrayList<TableRow> tblRows,
		...
		if (isQuestion && thisTitle == null) {
			...
			tblRow = tblRows.get(0);
			// return new RelativeLayout(con);
		} else if (isQuestion && thisTitle.equals("titleView")) {
			tblRow = tblRows.get(1);
		} else if (isQuestion && thisTitle.equals("answerView")) {
			tblRow = tblRows.get(2);
		} else {
			tblRow = tblRows.get(3);
		}
	...
	private View getQuestionView(int viewid) {
		...
		LinearLayout layout = new LinearLayout(con);
		double width = display.getWidth() - 20;
		LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
	...
	private ArrayList<Titles> altTitles;
	...
	public void setAltTties(ArrayList<Titles> newTitles) {
		this.altTitles=newTitles;
