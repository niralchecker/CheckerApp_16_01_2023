...
public class OldUITitleGroupHelper {
	...
	private int getWidth(Answers answers, int displayType, int qtl) {
		...
		if (isCheckbox) {
			...
			if (answers.getIconName() != null) {
				...
				cb.setText(Helper
						.makeHtmlString(ConvertTextCodetoText(ConvertTextCodetoText(answers
								.getAnswer()))));
			}
	...
	private View writeAnswersHorizontally(int displayType, int qtl,
		...
		for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
			...
			if (answers.getIconName() != null) {
				...
				cb.setId(ansCount + 1);
				cb.setTag(ConvertTextCodetoText(ConvertTextCodetoText(answers
						.getAnswer())));
