...
public class UIVerticalOldGroupHelper {
	...
	private View getCheckButtonImageView(ArrayList<Answers> listAnswers,
		...
		if (totalAnswers <= Constants.getAnswerCount(con)) {
			width = display.getWidth() * (62.0 / 100.0);
		} else {
			width = ((display.getWidth() * (62.0 / 100.0)) / Constants
		...
		for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
			...
			FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(Constants.getIconSize(),
					Constants.getIconSize(),
			...
			CustomCheckButtonClass rd = radioGroup.addRadioButton(false,
			...
			if (isPrev && isSelectedAnswers(qd, answers.getAnswerID()))
				radioGroup.setChecked(rd.radioIem, true);
			else
				radioGroup.setChecked(rd.radioIem, false);
	...
	private View getRadioButtonImageView(ArrayList<Answers> listAnswers,
		...
		for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
			...
			ImageView rd = radioGroup.addRadioButton(false,
	...
	private int getWidth(Answers answers, int displayType, int qtl) {
		...
		if (isCheckbox) {
			...
			if (answers.getIconName() != null) {
				...
				cb.setText(Helper.makeHtmlString(answers.getAnswer()));
			}
	...
	private View writeAnswersHorizontally(int displayType, int qtl,
		...
		for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
			...
			cb.setText(Helper.makeHtmlString(answers.getAnswer()));
			// cb.setText("2131423 2342lfnw dwfqrwf kWFMWPFM");
			...
			// if (toggleColor) {
			// color = R.drawable.tbl_border_grey_dark;
			//
			// }
	...
	private String[] getListItem(ArrayList<Answers> listAnswers,
		...
		if (listAnswers != null && listAnswers.size() > 0) {
			...
			for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
				...
				Spanned sp = Html.fromHtml(answers.getAnswer());
	...
	private View getDropDownView(ArrayList<Answers> listAnswers,
		...
		if (isPrev && qd != null) {
			...
			setSpinnerValue(Helper.getInt(questionObject.getObjectType()), qd, listAnswers,
	...
	private View getQuestionDescriptionView(boolean isReal) {
		TextView tv = new TextView(con);
		if (Helper.comapreString(questionObject.getAttachment(), "110"))
			IsAttachement = true;
		if (isReal)
			tv = setTextViewProperties(tv,
					questionObject.getQuestionDescription(),
			...
			tv = setTextViewProperties(tv, "", questionObject.getFont(),
					questionObject.getColor(), questionObject.getSize(),
					questionObject.getItalics(), "0",
					questionObject.getUnderline(), questionObject.getAlign());
	...
	private RelativeLayout.LayoutParams getEditTextLayoutParam(int id,
		...
		if (isNumber) {
			...
			RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(
					(int) (display.getWidth() * 62.0 / 100.0),
					display.getHeight() / 6);
			lp.addRule(setAlignmentOfView(Helper.getInt(questionObject
					.getAlign())));
			lp.addRule(RelativeLayout.BELOW, id);
	...
	public int getDateButtonEditTextView(Button btextbox, int type,
			boolean isPrev) {
	...
	public View prepareGroupLayout(Display display, Helper helper,
			...
			int questionNumber, String groupName) {
		RelativeLayout questionnaireLayout = new RelativeLayout(con);
		//if (listObjects==null || listObjects.size()==0) return questionnaireLayout;
		isRadioCheck = false;
		ultimateWidth = 0.0;
		...
		if (this.titles == null || this.titles.size() == 0) {
			tblRows = simpleTwoRowQuestions(listObjects.size() + 1, tblRows,
					MainLayout);
		} else {
		...
		for (objectCount = 0, viewOnPage = 1; objectCount < size; objectCount++, viewOnPage++) {
			...
			// if (questionObject.getDataID().contains("133987")
			// || questionObject.getDataID().contains("133988")) {
			// questionObject.setDisplayType("2");
			// }
			thisTitle = null;
			...
			try {
				...
				if (Helper.comapreString(questionObject.getObjectType(), "4")
					...
					tempId = viewId;
					View v = getQuestionDescriptionView(false);
					v.setTag(questionObject.getDataID());
					...
					if (questionObject.getListAnswers() != null
						...
						for (ansCount = 0; ansCount < questionObject
							...
							Answers answers = questionObject.getListAnswers()
									.get(ansCount);
							if (answers.getAnswerDisplayCondition() != null) {
								Boolean b = IsObjectdisplaybyCondition(answers
										.getAnswerDisplayCondition());
						...
						// lp.addRule(setAlignmentOfView(Helper.getInt(questionObject.getAlign())));
						lp.addRule(RelativeLayout.BELOW, tempId);
						questionnaireLayout.addView(vv, lp);
		...
		LinearLayout questionnaireLLayout = new LinearLayout(con);
		questionnaireLLayout.setOrientation(LinearLayout.VERTICAL);

		questionnaireLLayout.addView(
				getGroupNamevView(groupName,
						listObjects.get(0).getObjectCode(), viewId),
				new LayoutParams(LayoutParams.MATCH_PARENT,
						LayoutParams.WRAP_CONTENT));
		questionnaireLLayout.addView(hr, new LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));

		if (firstEditText != null)
			firstEditText.requestFocus();
		return questionnaireLLayout;
	}
	...
	private RelativeLayout addRow(int counter, ArrayList<TableRow> tblRows,
		...
		if (isQuestion && thisTitle == null) {
			...
			tmpLayout.addView(
					getGroupNamevView("##", listObjects.get(0).getObjectCode(),
							viewId), getGroupLayoutParam(0));
	...
	private View getQuestionView(int viewid) {
		...
		View dv = getQuestionDescriptionView(true);
	...
	private View getMiView(int type, boolean isPrev) {
		View v = getEditText(type, isPrev);
		thisQGroup.setTextbox(textbox);
		thisQGroup.setbTextbox(btextbox);
		if (order!=null && order.getIsJobInProgressOnServer()!=null
				&& order.isDataIdEnabled(set,questionObject.getDataID())) {
	...
	private void showTimePickerSeconds(final EditText txtTime) {
		...
		thisQGroup.setTextbox(txtTime);
		if (order!=null && order.getIsJobInProgressOnServer()!=null
				&& order.isDataIdEnabled(set,questionObject.getDataID())) {
			if (txtTime!=null) txtTime.setEnabled(false);
		}
		txtTime.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
