...
public class UIVerticalGroupHelper {
	...
	private View getCheckButtonImageView(ArrayList<Answers> listAnswers,
		...
		double width = display.getWidth() * (62.0 / 100.0);
		...
		for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
			...
			int color2 = Color.parseColor("#C0C0C0");
			int color1 = R.drawable.tbl_border_grey_dark;

			if (toggleColor) {
				color2 = Color.parseColor("#E0E0E0");
				color1 = R.drawable.tbl_border_grey_light;
	...
	private View writeAnswersHorizontally(int displayType, int qtl,
		...
		for (ansCount = 0; ansCount < listAnswers.size(); ansCount++) {
			...
			lpout.weight = 1;
			RelativeLayout view = new RelativeLayout(con);
			int color = R.drawable.tbl_border_grey_light;
	...
	private View getQuestionDescriptionView() {
		...
		tv.setTextSize(20);
		tv.setPadding(Constants.dpToPx(20),10,Constants.dpToPx(20),10);
	...
	private RelativeLayout tempHebrewQuestionLayout;
	...
	public View prepareGroupLayout(Display display, Helper helper,
		...
		for (objectCount = 0, viewOnPage = 1; objectCount < size; objectCount++, viewOnPage++) {
			...
			if (!isTitleDisplayShown
					&& (questionObject.getObjectDisplayCondition() == null || questionObject
				...
				questionObject.setObjectDisplayCondition("1=43");
					...
					&& questionObject.getObjectDisplayCondition().equals("1=2")) {
			...
			try {
				...
				if (!IsObjectdisplaybyCondition(questionObject
						.getObjectDisplayCondition()) || !isTitleDisplayShown) {
					questionObject.setObjectDisplayCondition("1=43");
					skipObjCount++;
					if (!DataID.equals("")) {
				...
				if (Helper.comapreString(questionObject.getObjectType(), "4")
					...
					if (modeSelect != 3) {
						...
						if (questionObject.getAttachment().equals("110")
							...
							TextView tv = commentTextView(con
									.getResources()
									.getString(
						...
						View v = getQuestionDescriptionView();
						 ...
						 getQuestionLayoutParam(viewId - 1));
		...
		hr.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT));
		hr.addView(MainLayout, new LayoutParams(LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT));
		...
		questionnaireLLayout.addView(
				getGroupNamevView(this.groupName, listObjects.get(0)
						.getObjectCode(), viewId), new LayoutParams(
