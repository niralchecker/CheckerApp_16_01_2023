...
public class NewUIQuestionGroupHelper {
...
//					questionObject.setObjectDisplayCondition("1=1");
				...
				if (Helper.comapreString(questionObject.getObjectType(), "4")
					...
					questionnaireLayout
							.addView(
									getGroupNamevView(groupName, listObjects.get(0)
	...
	private boolean ValidateForm2(Objects questionObject,
		...
		if (!Helper.comapreString(questionObject.getMandatory(), "1")
			...
			return true;
		} else { // qGroupThis.setMandatory(true);
			if (questionObject.getQuestionTypeLink() == null)
				return true;
			...
			if (Integer.parseInt(questionObject.getQuestionTypeLink()) == 8
					...
					&& qGroupThis.getCheckboxgrp() != null) {
				if (!isValidChackBoxChecked(
						questionObject.getMaxAnswersForMultiple(),
						qGroupThis.getCheckboxgrp())) {
					return false;
				}
			}
		}
	...
	private boolean isValidChackBoxChecked(String val, LinearLayout checkboxgrp) {
		if (val == null)
			return true;
		int answercount = 0;
		for (int i = 0; i < checkboxgrp.getChildCount(); i++) {
	...
	private boolean validateBlankAnswer(int qtype, int displayType,
		...
		case 7:
			if (displayType == 0 || displayType == 1)
				if (qGroupThis.getRadioGroup() != null)
					return qGroupThis.getRadioGroup().getCheckedRadioButtonId() != -1;
		...
		case 9:
			...
			if (qGroupThis.getCheckboxgrp() != null)
				return isChackBoxChecked(qGroupThis.getCheckboxgrp());
		}
		return false;
	}
			...
			if (IsObjectdisplaybyCondition(qGroups.get(i).getQuestionObject()
				...
				if (qGroups.get(i) == null
					...
					String bool = validateFreeTextBoxAnswer(qGroups.get(i)
							.getQuestionObject(), qGroups.get(i).getTextbox());
